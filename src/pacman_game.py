...
class PacmanGame:
    ...
    def __init__(self, game_map, pacman_position: List[int], ghosts_positions: List[List[int]]) -> None:
        ...
        if len(ghosts_positions) > 0:
            self.ghosts.append(HunterGhost(self, ghosts_positions[0], (self.rows, self.cols)))
        if len(ghosts_positions) > 1:
            self.ghosts.append(AmbusherGhost(self, ghosts_positions[1], (self.rows, self.cols)))
        if len(ghosts_positions) > 2:
            patrol_point = [1, 1] # Top-left corner
            self.ghosts.append(StrategicGhost(self, ghosts_positions[2], (self.rows, self.cols), patrol_point))
    ...
    def on_press(self, key):
        ...
        try:
            ...
            if key == keyboard.Key.up: move = 'up'
            elif key == keyboard.Key.down: move = 'down'
            elif key == keyboard.Key.left: move = 'left'
            elif key == keyboard.Key.right: move = 'right'
        
        if move and move in self.possible_moves and self.turn == 'pacman':
            try: self.move_queue.put_nowait(move)
            except queue.Full: pass

    def __get_possible_moves(self) -> List[str]:
        moves = []
        r, c = self.pacman_position
        if not self.is_movement_blocked(self.pacman_position, [r - 1, c]): moves.append('up')
        if not self.is_movement_blocked(self.pacman_position, [r + 1, c]): moves.append('down')
        if not self.is_movement_blocked(self.pacman_position, [r, c - 1]): moves.append('left')
        if not self.is_movement_blocked(self.pacman_position, [r, c + 1]): moves.append('right')
        return moves
    ...
    def print_game(self) -> None:
        ...
        if not self.output_map: return
        
        for row in self.output_map:
            line = []
            for char in row:
                line.append(char)
                if char not in ['😋', '👻']: line.append(' ')
    ...
    def __check_victory(self) -> bool:
        for row in self.tiles:
            for tile in row:
                if tile.has_pellet: return False
        return True
        ...
        try:
            ...
            while True:
                ...
                if self.turn == "ghost":
                    ...
                    _, output_map, pacman_position, lose = Pacman(self, self.output_map, self.pacman_position, move).move_pacman()
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                if tile.is_original_wall:
                    ...
                    if is_vertical and not is_horizontal: tile.display_char = '|'
                    elif is_horizontal and not is_vertical: tile.display_char = '-'
                    else: tile.display_char = '+'
                elif tile.has_pellet: tile.display_char = '.'
                else: tile.display_char = ' '
    ...
    def is_movement_blocked(self, from_pos: List[int], to_pos: List[int]) -> bool:
        if not self.in_bounds(to_pos): return True
    ...
    def remove_pellet(self, pos: List[int]) -> bool:
        if self.in_bounds(pos):
            ...
            if tile.has_pellet:
                ...
                if not tile.is_original_wall: tile.display_char = ' '
                return True
        return False
